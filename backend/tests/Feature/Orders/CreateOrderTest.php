...
class CreateOrderTest extends TestCase
{
    ...
    public function test_create_order_returns_correct_structure(): void
    {
        ...
        $response->assertCreated()
            ->assertJsonStructure([
                'id', 'company_uuid', 'label', 'status', 'number', 'sequence_id', 'created_at', 'updated_at',
            ]);
    }

    public function test_create_order_has_waiting_status_by_default(): void
    {
        $response = $this->postJson("/api/companies/{$this->company->id}/orders", ['label' => 'Açaí']);
        $response->assertCreated()->assertJsonFragment(['status' => 'waiting']);
    }

    public function test_create_order_persists_in_database(): void
    {
        $this->postJson("/api/companies/{$this->company->id}/orders", ['label' => 'Tapioca']);
        $this->assertDatabaseHas('orders', ['company_id' => $this->company->id, 'label' => 'Tapioca']);
    }
    ...
    {
        $response = $this->postJson("/api/companies/{$this->company->id}/orders", []);
        $response->assertUnprocessable()->assertJsonValidationErrors(['label']);
    }

    public function test_create_order_with_nonexistent_company_returns_404(): void
    {
        $response = $this->postJson('/api/companies/zzzzzz/orders', ['label' => 'Test']);
